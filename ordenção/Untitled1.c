...
int main(int argc, char *argv[]){
    ...
    TipoItem item;
    FILE *input, *output; //arquivo que lera as entradas
	...
	if ((input = fopen(nome, "r")) == NULL){
		...
		if ((output = fopen("out.dat", "wb")) == NULL){
		printf("Erro ao abrir o arquivo.\n");
		return 0;
	}

        while(fscanf(input,"%s", item.Nome) != EOF){
            fscanf(input,"%d", &item.Nota);
            Empilha(item,&vet[item.Nota]);
            fwrite(&item, sizeof(TipoItem), 1, output);
            //printf("%s\n %d\n", item.Nome, item.Nota);
            }
    for(i=0;i<11;i++){
        while(!Vazia(vet[i])){
